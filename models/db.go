...
type Users struct {
	...
	Mtime  int64 `gorm:"column:mtime;type:int(11)" description:"修改时间"`
	...
	Status int64       `gorm:"column:status;type(10)" json:"status" description:"用户状态，1(启用)/2(禁用)/3(删除)"`
}
...
func (u *Users) CheckuserStatus() (bool,bool,bool,error) {
	has,err := u.IsExist()
	if err != nil {
...
func (u *Users) IsExist() (bool,error) {
	tx,err := u.getRequirement()
	if err != nil {
		return true,err
	}
...
func (u *Users) IsDisable() (bool,error) {
	tx,err := u.getRequirement()
	if err != nil {
		return true,err
	}
	err = tx.Where("status = 2").Take(&u).Error
	if errors.Is(err, gorm.ErrRecordNotFound) {
		tx.Rollback()
	...
	/*
	err := db.Table(USERS).Where("en_name = ? and status = 2 or status = 3",u.EnName).Take(&u).Error
	if errors.Is(err, gorm.ErrRecordNotFound) {
		return false,nil
	} else if err != nil {
		return false,err
...
func (u *Users) Get() error {
	err := db.Table(USERS).Where("en_name = ?",u.EnName).Take(&u).Error
	if errors.Is(err, gorm.ErrRecordNotFound) {
