...
func Login(c *gin.Context, param models.ParamLogin) (string,string, error) {
	...
	case "verify":
		...
		if err != nil || user.Status != 1 {
			middleware.LogErr(c).Errorf(fmt.Sprintf("user %v status unusual", param.Mobile))
			return "",fmt.Sprintf("用户状态异常,请联系管理员查询！"),errors.New("用户状态异常,请联系管理员查询！")
		}
		//验证手机号是否合法
		if toolkits.CheckMobile(param.Mobile) {
			middleware.LogErr(c).Errorf(fmt.Sprintf("mobile %v invalid", param.Mobile))
			return "",fmt.Sprintf("手机号不合法"),errors.New(" 手机号不合法")
		}
		//验证码校验
		result,err := pkg.SmsCheck(param.Mobile, param.VerifyCode) 
		if err != nil {
			middleware.LogErr(c).Errorf(fmt.Sprintf("mobild %v verify code check failed:%v",param.Mobile,err))
			return "",fmt.Sprintf("验证码校验失败，请联系管理员排查"),err
		}
		if !result {
			middleware.LogErr(c).Errorf(fmt.Sprintf("mobile %v verify code %v invalid", param.Mobile, param.VerifyCode))
	...
	case "account":
		...
		err := user.GetByUname()
		if err != nil || user.Status != 1 {
			middleware.LogErr(c).Errorf(fmt.Sprintf("user %v status unusual", user.EnName))
			return "",fmt.Sprintf("用户状态异常,请联系管理员查询！"),errors.New("用户状态异常,请联系管理员查询！")
		}
		decryptpwd, err := pkg.Decrypt(user.Password, config.Conf.Util.InitKey)
		if err != nil {
			middleware.LogErr(c).Errorf(fmt.Sprintf("user authentication failed:%v", err))
			return "",fmt.Sprintf("用户身份验证失败,请联系系统管理员查看"),err 
		}
		if param.PassWord != string(decryptpwd) {
			middleware.LogErr(c).Errorf(fmt.Sprintf("user passwd :%v invalid,right pwd:%v", param.PassWord, string(decryptpwd)))
			return "",fmt.Sprintf("密码不正确"),errors.New ("密码不正确")
		}
	default:
		middleware.LogErr(c).Errorf(fmt.Sprintf("login type %v invalid", param.Type))
	...
	if err != nil {
		middleware.LogErr(c).Errorf(fmt.Sprintf("user :%v login failed:%v\n", user.EnName))
		return "",fmt.Sprintf("登录失败"),err 
	}
	middleware.LogInfo(c).Infof(fmt.Sprintf("user :%v login success", user.EnName))
...
func CheckUseLoginByUname(c *gin.Context,param models.ParamUserEnName) (string,string,error) {
	...
	if err != nil || val == "" {
		middleware.LogErr(c).Errorf(fmt.Sprintf("user %v not login",param.EnName))
...
func SendSms(c *gin.Context,param models.ParamMobile) (string, error) {
	...
	if err != nil {
		middleware.LogErr(c).Errorf(fmt.Sprintf("send sms to mobile %v failed:%v\n", param.Mobile, err))
		return fmt.Sprintf("短信验证码发送失败"),err 
	}

	err = pkg.SmsSet(param.Mobile, code, 60)
	if err != nil {
		middleware.LogErr(c).Errorf(fmt.Sprintf("mobile %v set sms code %v to redis failed:%v\n", param.Mobile, code, err))
		return fmt.Sprintf("短信验证码发送失败"),err 
	}
